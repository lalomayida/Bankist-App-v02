...
const account1 = {
  ...
  movementsDates: [
    '2022-02-02T21:31:17.178Z',
    '2022-02-01T07:42:02.383Z',
    '2022-01-31T09:15:04.904Z',
...
const displayMovements = function (acc, sort = false) {
  ...
  movs.forEach(function (mov, i) {
    ...
    const dislpayDate = formatMovementDate(date, acc.locale);
    const formattedMov = formatCur(mov, acc.locale, acc.currency);
    const html = `
      <div class="movements__row">
        <div class="movements__type movements__type--${type}">
        ${i + 1} ${type}</div>
        <div class="movements__date">${dislpayDate}</div>
...
const calcDisplaySummary = function (acc) {
  ...
  const out = acc.movements
    .filter(mov => mov < 0)
    .reduce((acc, mov) => acc + mov, 0);
  const formattedOutcomes = formatCur(Math.abs(out), acc.locale, acc.currency);
